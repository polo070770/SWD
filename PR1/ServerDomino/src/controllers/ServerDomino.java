...
public class ServerDomino extends Domino {
	...
	public void initGame(){
		...
		while(STATE == State.PLAYING){
			...
			if(STATE != State.ENDGAME){ // si no se ha terminado el juego, esperamos la siguiente accion del cliente
	...
	private void analizeMovement(){
		...
		if(rivalMove.isNT()){ // cliente no puede tirar
			this.STATE = State.CLIENTNP; // el estado pasa a ser que el cliente no ha podido tirar
			
			if(this.remainingPile.getLength() > 0){ // aun hay piezas para robar
				Piece piece = this.remainingPile.getRandomPiece(); //extraemos una pieza de la pila de pendientes
				givePiece(piece);
			}else{ // no hay piezas para robar, miramos movimiento cliente
				
				if(this.player.hasMove(this.playedPile)){ // servidor puede tirar
					makeMovement();
			...
			if(this.isValidMovement(rivalMove)){// comprobamos si es una accion valida del cliente
				...
				if(this.player.hasMove(this.playedPile)){ // servidor puede tirar
					...
					if(!this.player.hasMove(this.playedPile)){ // si aun no puede tirar
						makeNoMovement();
	...
	private void makeMovement(){
		...
		this.comm.sendServerMovement(newMove, this.player.handLength(), this.remainingPile.getLength());
		
	}
	
	private void makeNoMovement(){
		if(this.STATE == State.CLIENTNP || this.STATE == State.SERVERNP){ // cliente no puede tirar y servidor tampoco
			this.STATE = STATE.ENDGAME;
			//TODO NEXT ACTION
		}else{
