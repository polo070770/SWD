	...
	public void run() {
		if (connected){
			...
			if(validHandShake){
				...
				// una vez hemos recibido el mensaje inicial del cliente, y siendo este valido, iniciamos el juego
				ServerDomino game = new ServerDomino(comm);
				game.initGame();
				System.out.println(socket.getInetAddress() + ":" + socket.getPort() + " disconnected");
				disconnect();
			}else{
				System.out.println(socket.getInetAddress() + ":" + socket.getPort() + " refused");
				disconnect();
